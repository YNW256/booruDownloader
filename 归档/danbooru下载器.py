...
print('\n\033[1;31mdanbooru下载器v0.0.9\033[0m'+' by Youngwang')
print('\n当前下载文件夹路径：'+PATH)
...
if len(pagelist) == 0 :
    imglist=re.findall(r'alt=\"post \#(.+?)\"',response.text) # 判断是否只有一页
    if len(imglist) == 0 :
        print('无搜索结果')
        sys.exit()
    else:
    ...
    print('共搜索到'+maxpage+'页（该网站最多可访问1000页）')
...
for page in range(int(down),int(up)): # 循环，下载某页图片
    ...
    for x in imglist: # 循环，下载某张图片
        ...
        # 访问某张图片----------------------------------------------
        url='https://danbooru.donmai.us/posts/'+x
